...
if [ "$TRAVIS_BRANCH" = 'master' ] && [ "$TRAVIS_PULL_REQUEST" == 'false' ]; then
    ...
    openssl aes-256-cbc -K $encrypted_a3badead09c1_key -iv $encrypted_a3badead09c1_iv -in scripts/continuous-integration/codesigning.asc.enc -out scripts/continuous-integration/codesigning.asc -d
